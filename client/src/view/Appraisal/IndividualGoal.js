...
export default function IndividualGoal(props) {
  ...
  const token = auth.token;   
    ...
    <div className="goals_report_view">
        ...
        {props.MyAppraisal?.map((res, index)=>{ 
              var clName = "";
              var checked = false;
              if(res.goalId == props.selectedGoal && res.tacticId == props.selectedTactic){
                clName = "selected";
                checked = true;
              }
      ...
      <div className="pagination_wrp">
        {(props.MyAppraisal.length > 0 ?
        <div className="pagination_wrp"> 
        <Pagination itemsPerPage={5} items={props.MyAppraisal} totalRecord={props.totalRecordsData} method={props.methods}/>
